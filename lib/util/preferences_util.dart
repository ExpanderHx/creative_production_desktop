...
class PreferencesUtil{
  //创建工厂方法
  static PreferencesUtil? _instance;
  factory PreferencesUtil() => _instance ??= PreferencesUtil._initial();
  SharedPreferences? _preferences;
  //创建命名构造函数
  PreferencesUtil._initial() {
  ...
  static Future<PreferencesUtil?> perInit() async {
    if (_instance == null) {
      //静态方法不能访问非静态变量所以需要创建变量再通过方法赋值回去
      SharedPreferences preferences = await SharedPreferences.getInstance();
      _instance = PreferencesUtil._pre(preferences);
    }
    return _instance;
  }
  PreferencesUtil._pre(SharedPreferences prefs) {
    _preferences = prefs;
  }
  ///设置String类型的
